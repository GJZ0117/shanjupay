...
public class MerchantConvertImpl implements MerchantConvert {
    ...
    @Override
    public List<MerchantDTO> entityList2dtoList(List<Merchant> merchantList) {
        if ( merchantList == null ) {
            return null;
        }

        List<MerchantDTO> list = new ArrayList<MerchantDTO>( merchantList.size() );
    ...
    @Override
    public List<Merchant> dtoList2entityList(List<MerchantDTO> merchantDTOList) {
        if ( merchantDTOList == null ) {
            return null;
        }

        List<Merchant> list = new ArrayList<Merchant>( merchantDTOList.size() );
        for ( MerchantDTO merchantDTO : merchantDTOList ) {
            list.add( dto2entity( merchantDTO ) );
        }
