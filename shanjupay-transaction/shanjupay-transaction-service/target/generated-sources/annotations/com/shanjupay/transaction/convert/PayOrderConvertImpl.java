...
public class PayOrderConvertImpl implements PayOrderConvert {
    ...
    @Override
    public List<PayOrderDTO> listentity2dto(List<PayOrder> payOrders) {
        if ( payOrders == null ) {
            return null;
        }

        List<PayOrderDTO> list = new ArrayList<PayOrderDTO>( payOrders.size() );
        for ( PayOrder payOrder : payOrders ) {
            list.add( entity2dto( payOrder ) );
        }
    ...
    @Override
    public List<PayOrder> listdto2entity(List<PayOrderDTO> payOrderDTOS) {
        if ( payOrderDTOS == null ) {
            return null;
        }

        List<PayOrder> list = new ArrayList<PayOrder>( payOrderDTOS.size() );
        for ( PayOrderDTO payOrderDTO : payOrderDTOS ) {
            list.add( dto2entity( payOrderDTO ) );
        }
